...
def dfs(place, dist_so_far, roads, distances):
    ...
    """
    if not (place in distances):
        distances[place] = 0
    for i in roads[place]:
        if not (i[0] in distances):
            distances[i[0]]=dist_so_far+i[1]
        else:
            if dist_so_far+i[1]<distances[i[0]]:
                distances[i[0]]=dist_so_far+i[1]
    for i in roads[place]:
        for j in roads[i[0]]:
            if not (j[0] in distances):
                dfs(i[0],distances[i[0]],roads,distances)
            else:
                if distances[i[0]]+j[1]<distances[j[0]]:
                    distances[j[0]]=distances[i[0]]+j[1]
                    dfs(i[0],distances[i[0]],roads,distances)
