...
Authors: Jim Li
Consulted with: Linze Li. Refer to his codes.
        ...
        for group in groups:
            ...
            for tile in group:
                if tile.symbol == sdkboard.OPEN:
                    continue
                elif tile.symbol in possible:
                    possible.remove(tile.symbol)
                else:
                    duplicate = True
                    tile.announce("duplicate")
                    for recheck in group:
                        if tile.symbol == recheck.symbol:
                            recheck.announce("duplicate")
        if duplicate:
            return False
        else:
            return True
        ...
        for digit in possible:
            ...
            for tile in group:
                if digit in tile.possible:
                    ...
                    row = tile.row
            if count == 1:
                groups[row][col].determine(digit)
