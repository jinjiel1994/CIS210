...
def getRoute(map,step,adm_x,adm_y,alien_x,alien_y):
    ...
    setLoc(map,adm_x,adm_y,"A")  

    step=step+1

    # You'll need to write the recursive calls and return

    if not (getRoute(map,step,adm_x,adm_y-1,alien_x,alien_y)==None):
        return "N" + getRoute(map,step,adm_x,adm_y-1,alien_x,alien_y)
    if not (getRoute(map,step,adm_x,adm_y+1,alien_x,alien_y)==None):
        return "S" + getRoute(map,step,adm_x,adm_y+1,alien_x,alien_y)
    if not (getRoute(map,step,adm_x-1,adm_y,alien_x,alien_y)==None):
        return "W" + getRoute(map,step,adm_x-1,adm_y,alien_x,alien_y)
    if not (getRoute(map,step,adm_x+1,adm_y,alien_x,alien_y)==None):
        return "E" + getRoute(map,step,adm_x+1,adm_y,alien_x,alien_y)
