...
def getRoute(map,step,adm_x,adm_y,alien_x,alien_y,route):
    ...
    if getRoute(map,step+1,adm_x,adm_y-1,alien_x,alien_y,route):
        route.append("N")
        return True
    if getRoute(map,step+1,adm_x,adm_y+1,alien_x,alien_y,route):
        route.append("S")
        return True
    if getRoute(map,step+1,adm_x-1,adm_y,alien_x,alien_y,route):
        route.append("W")
        return True
    if getRoute(map,step+1,adm_x+1,adm_y,alien_x,alien_y,route):
        route.append("E")
...
def main():
    ...
    route=list()
    if getRoute(map,0,admx,admy,alnx,alny,route):
        map=readMap(mapfile)
